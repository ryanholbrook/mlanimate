...
sigma_0 <- matrix(c(2, 0.5, 0.5, 1), nrow = 2)
mu_1 <- c(2, 0)
sigma_1 <- matrix(c(0.5, -0.5, -0.5, 1), nrow = 2)
df_1 <- 5
n <- 4000
set.seed(31415)
sample_mvt <- make_sample_mvt(n,
                              mu_0, sigma_0, df_0,
                              mu_1, sigma_1, df_1,
                              p)
density_mvt <- make_density_mvt(mu_0, sigma_0, df_0,
                                mu_1, sigma_1, df_1,
                                p,
                                -3, 5, -3, 5)
(ggplot() +
 gg_sample(sample_mvt, alpha = 0.25) +
 gg_optimal(density_mvt)) +
(ggplot() +
 gg_density(density_mvt, z = p_0_xy,
            alpha = 0.25) +
 gg_density(density_mvt, z = p_1_xy,
            alpha = 0.25) +
 gg_optimal(density_mvt)) +
plot_annotation("The Optimal Decision Boundary")

fit_mvt_nb <- naivebayes::naive_bayes(factor(class) ~ x + y, data = sample_mvt)
pred_mvt_nb <- predict(fit_mvt_nb, newdata = density_mvt[, c("x", "y")], type = "prob")
density_mvt_nb <- cbind(density_mvt, "fitted" = pred_mvt_nb[, "1"])
gg_plot_boundary(sample_mvt, density_mvt_nb, title = "Naive Bayes")

fit_mvt_qda <- MASS::qda(class ~ x + y, data = sample_mvt)
pred_mvt_qda <- predict(fit_mvt_qda, newdata = density_mvt)
density_mvt_qda <- cbind(density_mvt, "fitted" = pred_mvt_qda$posterior[, "1"])
gg_plot_boundary(sample_mvt, density_mvt_qda, title = "QDA")
